...
function update_whether()
    local success, stdout, stderr = wezterm.run_child_process {'curl', '--silent',
                                                               'https://api.open-meteo.com/v1/forecast?latitude=35.7&longitude=139.82&daily=weathercode,temperature_2m_max,temperature_2m_min&forecast_days=3&timezone=Asia%2FTokyo'} -- At Tokyo SkyTree
...
function create_powerlines(window, pane)
    local weather_infos = zip(wezterm.GLOBAL.weathercodes, wezterm.GLOBAL.max_temperatures,
        wezterm.GLOBAL.min_temperatures)
    local styled_whethers = enumerate(weather_infos, function(weather_info, index)
        return styled_whether(table.unpack(weather_info))
    end)
    local styled_texts = {styled_whethers[1], styled_whethers[2], styled_whethers[3], {{
        Foreground = {
            Color = "#c0c0c0"
        }
    }, {
        ...
        Text = string.format(" %s(%s) %s ", wezterm.strftime("%-m/%-d"), day_of_week_in_japan(wezterm.strftime("%u")),
